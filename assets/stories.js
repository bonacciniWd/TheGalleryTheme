document.addEventListener("DOMContentLoaded", () => {
  ...
  function showStory(index) {
    ...
    console.log("DEBUG: Valor de data-external-link lido pelo JS:", currentShareExternalUrl); // Mantenha este log aqui

    modalTitle.textContent = title;
    // modalMedia.innerHTML = ""; // Já limpamos no início da função
    ...
    if (videoUrl) {
      ...
      video.onloadedmetadata = () => {
        ...
        // Tenta reproduzir o vídeo. Isso retorna uma Promise.
        video.play().then(() => {
          console.log("[VIDEO EVENT] Vídeo começou a reproduzir com sucesso!");
          // Somente AGORA o timer do story deve começar
          startProgress(video.duration * 1000);
          timer = setTimeout(() => {
            showStory(currentIndex + 1);
          }, video.duration * 1000);
        }).catch(error => {
          // Captura erros da Promise de play() (ex: NotAllowedError se autoplay for bloqueado)
          console.error("[VIDEO ERROR] Erro ao tentar reproduzir o vídeo após loadedmetadata:", error);
          if (error.name === 'NotAllowedError') {
              console.warn("[VIDEO WARN] Autoplay bloqueado pelo navegador (iOS?). Usuário precisa interagir.");
              // Opcional: Aqui você pode mostrar um botão de play para o usuário clicar
              // Por enquanto, vamos pular para o próximo, pois o vídeo não tocará.
          }
          showStory(currentIndex + 1); // Pula para o próximo story em caso de falha de reprodução
        });
      };
      ...
      video.onerror = (e) => {
        ...
        // Pula para o próximo story em caso de erro de carregamento
        showStory(currentIndex + 1);
      };
