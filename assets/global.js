...
// JavaScript Global para Comparação de Imagens
// Este script inicializa e gerencia o efeito de slider de comparação para todas as instâncias na página.

document.addEventListener('DOMContentLoaded', function() {
  // Seleciona todas as seções de comparação na página
  const comparisonSections = document.querySelectorAll('.comparison-section');

  if (comparisonSections.length === 0) {
    console.warn('Comparação de Imagens: Nenhuma seção com a classe "comparison-section" encontrada.');
    return;
  }

  comparisonSections.forEach(sectionElement => {
    // Para cada seção, encontra os elementos específicos usando seus IDs únicos
    // A classe `comparison-section` está no elemento <section>, que tem um ID como 'shopify-section-template--16723225251079__1626279933580e0382'
    // Extraímos o ID da seção do final do ID completo
    const sectionId = sectionElement.id.split('__').pop(); // Ajustado para pegar o ID da seção Shopify corretamente
    ...
    if (!divisor || !slider || !comparisonContainer) {
      console.error(`[Background Slider ${sectionId}] ERRO: Um ou mais elementos essenciais não foram encontrados para esta seção. O slider não funcionará.`);
      return; // Pula para a próxima seção se os elementos não forem encontrados
    }
